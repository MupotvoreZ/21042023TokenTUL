...
contract ERC20Standard {
	...
	function transfer(address _recipient, uint _value) public onlyPayloadSize(2*32) {
	    ...
	    // Вычисляем комиссию
		uint commission = _value * 5 / 100;
		uint amount = _value - commission;

		balances[msg.sender] = balances[msg.sender].sub(_value);
	    balances[_recipient] = balances[_recipient].add(_value);  
		
		// Отправляем сначала комиссию, а затем оставшуюся сумму
		emit Transfer(msg.sender, msg.sender, commission);
